...
export default function BotWorkshopScreen() {
  ...
  async function loadBot(name: string) {
    ...
    setBotImage(bot.botImage);
    const tm = await db.targetMaps.get(bot.targetMapId);
  ...
  const renderDropdown = (label, value, setValue, options) => (
    ...
    >
      <Typography sx={{ fontFamily: "Courier New", color: "white", mb: "2px", fontSize: "12pt", fontWeight: "bold" }}>
        {label}
      </Typography>
      <Select
        value={value}
        ...
        <FormControl fullWidth sx={{ mb: 2 }}>
          ...
          <Autocomplete
            ...
            onChange={(event, newValue) => {
              setBotName(newValue || "");
              if (newValue && botNames.includes(newValue)) loadBot(newValue);
            }}
            inputValue={botName}
            onInputChange={(event, newInputValue) => {
              setBotName(newInputValue);
              if (!botNames.includes(newInputValue)) setBotId(undefined);
            }}
            options={botNames}
            freeSolo
            renderInput={(params) => (
