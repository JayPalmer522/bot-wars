...
Node.js requires installing a lightweight HTTP server package:
...
export async function BEGIN_COMBAT(army1Id, army2Id, settings = {}) {
  ...
  let TEST_VALUE = "JAY_1";
    window.alert("FUNCTION BEGIN_COMBAT - START");
...
//  window.alert(
//    "TEST 3\n" +
//    " army1Data: " + JSON.stringify(await db.armies.get(army1Id)) + "\n" +
//    " army2Data: " + JSON.stringify(await db.armies.get(army2Id)) + "\n" +
//    " army1: " + JSON.stringify(await db.armies.get(army1)) + "\n" +
//    " army2: " + JSON.stringify(await db.armies.get(army2)) + "\n" +
//    " army1Data.id: " + army1Id + "\n" +
//    " army2Data.id: " + army2Id + "\n" +
//    " army1: " + army1 + "\n" +
//    " army2: " + army2 + "\n" +
...
//  );
  ...
  console.log(`ALL_BOTS_IN_COMBAT_LIST${JSON.stringify(ALL_BOTS_IN_COMBAT_LIST)}`);
    ...
    const STILL_FIGHTING = MAIN_COMBAT_LOOP(ALL_BOTS_IN_COMBAT_LIST);
    window.alert("FUNCTION BEGIN_COMBAT - END");
...
export async function MAIN_COMBAT_LOOP(ALL_BOTS_IN_COMBAT_LIST) {
  ...
  // Placeholder - will process turns in sequence
     window.alert("FUNCTION MAIN_COMBAT_LOOP - START");
 
  
  let TOO_MANY_TURNS = 0 ;
  let BOT_IS_DESTROYED = 0 ;
  
    const STILL_FIGHTING = UPDATE_MAP_SQUARES(ALL_BOTS_IN_COMBAT_LIST);

     window.alert("FUNCTION MAIN_COMBAT_LOOP - END");
  return { success: true };
}
 ...
 */
export function UPDATE_MAP_SQUARES(ALL_BOTS_IN_COMBAT_LIST) {
  console.log("UPDATE_MAP_SQUARES called");
	window.alert("FUNCTION UPDATE_MAP_SQUARES - START");
  
     window.alert("***OBJECTS: " + ALL_BOTS_IN_COMBAT_LIST);
  ...
  try {
    ...
    const ALL_BOTS_IN_COMBAT_LIST_TEMP = [JSON.stringify(ALL_BOTS_IN_COMBAT_LIST)];

		   window.alert("XXXXXXXXXXXXXXXXX ");
let MyCount = 0;
let CURRENT_GAME_MAP = 0;
      // For each bot position in the list
      for (const OBJECT_TEMP of ALL_BOTS_IN_COMBAT_LIST) {
		MyCount = MyCount +1;

		console.log(OBJECT_TEMP[name]); 
		let NEXT_BOT =(JSON.stringify(OBJECT_TEMP));
//		console.log(OBJECT_TEMP.name); 
//		window.alert(".MyCount." + MyCount + ".NAME." + OBJECT_TEMP.name + ".BOT." + OBJECT_TEMP.combatBotNumber + ".ARMY." + OBJECT_TEMP.army + ".MOVE." + OBJECT_TEMP.move + "..." + "NEXT_BOT: " + NEXT_BOT);
		if (MyCount == 1){
		window.alert(".MyCount: " + MyCount);
		CURRENT_GAME_MAP ===============================
		} //if 
		else if (MyCount == 2){
		window.alert(".MyCount: " + MyCount);
		...
		window.alert(" Add a zero at: " + MyCount);
		}
	
	} //for


	window.alert("FUNCTION UPDATE_MAP_SQUARES - END");
...
export function CHECK_BOT_TOTALS(frame, engine, computer, armor, sensor, weaponMaster, weaponSecondary, weaponBomb) {
  console.log("CHECK_BOT_TOTALS called");
    window.alert("FUNCTION CHECK_BOT_TOTALS - START");
  ...
  return {
    ...
    window.alert("FUNCTION CHECK_BOT_TOTALS - END");
...
export async function GET_VALUES_FROM_RECORDS(currentBot, armyNumber, currentBotNumber) {
  console.log(`GET_VALUES_FROM_RECORDS called for bot: ${currentBot.name}, army: ${armyNumber}`);
    window.alert("FUNCTION GET_VALUES_FROM_RECORDS - START");
  ...
  try {
    ...
    window.alert("FUNCTION GET_VALUES_FROM_RECORDS - END");
  }
}
...
export async function CURRENT_ACTIVATE_SCANNER_FULL(combatState) {
  console.log("CURRENT_ACTIVATE_SCANNER_FULL called");
    ...
    window.alert("FUNCTION CURRENT_ACTIVATE_SCANNER_FULL - START");
  try {
...
export function CREATE_ALL_BOTS_IN_COMBAT_LIST(army1Bots = [], army2Bots = []) {
  // Generate random number (1 or 2) to decide which army goes first
  const firstArmy = Math.floor(Math.random() * 2) + 1; // 1 or 2
    window.alert("FUNCTION CREATE_ALL_BOTS_IN_COMBAT_LIST - START");

  // Shuffle each army separately using Fisher-Yates algorithm
  const shuffledArmy1 = [...army1Bots];
  for (let i = shuffledArmy1.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [shuffledArmy1[i], shuffledArmy1[j]] = [shuffledArmy1[j], shuffledArmy1[i]];
  }

  const shuffledArmy2 = [...army2Bots];
  for (let i = shuffledArmy2.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [shuffledArmy2[i], shuffledArmy2[j]] = [shuffledArmy2[j], shuffledArmy2[i]];
  }

  const allBots = [];
  const maxLength = Math.max(shuffledArmy1.length, shuffledArmy2.length);

  // Add bots alternately by army number
  for (let i = 0; i < maxLength; i++) {
    if (firstArmy === 1) {
      // Army 1 first: army1, army2, army1, army2, ...
      if (i < shuffledArmy1.length) {
        allBots.push({ ...shuffledArmy1[i], army: 1 });
      }
      if (i < shuffledArmy2.length) {
        allBots.push({ ...shuffledArmy2[i], army: 2 });
      }
    } else {
      // Army 2 first: army2, army1, army2, army1, ...
      if (i < shuffledArmy2.length) {
        allBots.push({ ...shuffledArmy2[i], army: 2 });
      }
      if (i < shuffledArmy1.length) {
...
//  let ALL_BOTS_IN_COMBAT_LIST_TEMP=(JSON.stringify(ALL_BOTS_IN_COMBAT_LIST))
    window.alert("FUNCTION CREATE_ALL_BOTS_IN_COMBAT_LIST - END");
